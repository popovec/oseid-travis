...
prepare_ec_param (struct ec_param *c, ec_point_t * p, uint8_t size)
{
  ...
  if (size == 0)
	...
	return 0;
      ...
      if (ret !=
	  fs_key_read_part ((uint8_t *) & c->working_key, KEY_EC_PRIVATE))
	return 0;
    }
  else
    ret = size;
...
static uint8_t
sign_ec_raw (uint8_t * message, struct iso7816_response *r, uint16_t size)
{
#if MP_BYTES > 48
...
uint8_t
security_env_set_reset (uint8_t * message)
{
  uint16_t tagval;
  uint8_t xlen, *data;;
  uint8_t tag;
...
// is security enviroment set to sign ?
  if ((sec_env_valid &
      ...
      (SENV_TEMPL_DST | SENV_FILE_REF | SENV_REF_ALGO))
    {
      ...
      DPRINT ("sec env %02x\n", sec_env_valid);
      DPRINT ("mask %02x\n",
	      SENV_TEMPL_MASK | SENV_ENCIPHER | SENV_FILE_REF |
	      SENV_REF_ALGO);
      DPRINT ("To:  %02x\n", SENV_TEMPL_DST | SENV_FILE_REF | SENV_REF_ALGO);
      return S0x6985;		//    Conditions not satisfied
    }
